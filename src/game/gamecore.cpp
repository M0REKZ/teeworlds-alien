...
{
	pObjCore->m_X = teeround(m_Pos.x);
	pObjCore->m_Y = teeround(m_Pos.y);

	pObjCore->m_VelX = teeround(m_Vel.x*256.0f);
	pObjCore->m_VelY = teeround(m_Vel.y*256.0f);
	pObjCore->m_HookState = m_HookState;
	pObjCore->m_HookTick = m_HookTick;
	pObjCore->m_HookX = teeround(m_HookPos.x);
	pObjCore->m_HookY = teeround(m_HookPos.y);
	pObjCore->m_HookDx = teeround(m_HookDir.x*256.0f);
	pObjCore->m_HookDy = teeround(m_HookDir.y*256.0f);
