...
bool CNetServer::Open(NETADDR BindAddr, CNetBan *pNetBan, int MaxClients, int MaxClientsPerIP, int Flags)
{
	...
	// 0.0.0.0 would never connect as client
	net_addr_from_str(&m_LastConAddr,"0.0.0.0");
	net_addr_from_str(&m_PrevLastConAddr,"0.0.0.0");
			...
			else
			{
				...
				if(m_RecvUnpacker.m_Data.m_Flags&NET_PACKETFLAG_CONTROL && m_RecvUnpacker.m_Data.m_aChunkData[0] == NET_CTRLMSG_CONNECT)
				{
					...
					if(!Found)
					{
						...
						if((!net_addr_comp(&ThisAddr, &m_LastConAddr) && time_get() - m_LastConTry < 0)
								 && (!net_addr_comp(&ThisAddr, &m_PrevLastConAddr) && time_get() - m_PrevLastConTry < 0))
						{
							char aBuf[128];
							str_format(aBuf, sizeof(aBuf), "Too fast reconnect.");
							CNetBase::SendControlMsg(m_Socket, &Addr, 0, NET_CTRLMSG_CLOSE, aBuf, sizeof(aBuf));
							return 0;
						}

						m_PrevLastConAddr = m_LastConAddr;
						m_PrevLastConTry = m_LastConTry;

						m_LastConAddr = ThisAddr;
						m_LastConTry = time_get() + 3 * time_freq();
